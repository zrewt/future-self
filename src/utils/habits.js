...
/**
 * NEW — per-tracking-type increment size for the +/- buttons.
 * Fixes a bug where "times" (e.g. veg servings) was using a generic
 * step of 5 instead of 1, making a 3-serving target unreachable by
 * single taps and confusing to use.
 */
export function getHabitStep(habit) {
  switch (habit.tracking_type) {
    case 'distance': return 0.5
    case 'times':    return 1
    case 'amount':   return (habit.target_value && habit.target_value >= 500) ? 250 : 50
