...
import {
  ...
  const DIMENSIONS = [
    { key: 'nutrition', label: 'Nutrition', calc: (log) => calcNutritionFromServings(log, []) },
    { key: 'fitness',   label: 'Fitness',   calc: (log) => calcFitnessFromWorkout(log) },
    { key: 'sleep',     label: 'Sleep',     calc: (log) => calcSleepScore(log) },
    { key: 'hydration', label: 'Hydration', calc: (log) => calcHydrationScore(log) },
    { key: 'habits',    label: 'Habits',    calc: (log) => calcHabitsScore(log) },
  ]
  ...
  export function projectFutureSelf(logs, actualCurrentFSS, currentStreak = 0) {
    ...
    DIMENSIONS.forEach((d) => {
      ...
      dimensionsToday[d.key] = clamp(intercept + slope * xToday)
    })
  
    const streakToday = currentStreak
    const scoreRegToday = calcFutureSelfScore(dimensionsToday, streakToday)
    ...
    HORIZONS.forEach((h) => {
      ...
      DIMENSIONS.forEach((d) => {
        ...
        const extra = slope * h.days * dampen(h.days)
        dimensionsFuture[d.key] = clamp(dimensionsToday[d.key] + extra)
      })
  
      const streakFuture = Math.min(100, currentStreak + h.days)
      const scoreRegFuture = calcFutureSelfScore(dimensionsFuture, streakFuture)
      ...
      // Use the longest horizon's per-dimension deltas as the "what's
      // driving this" breakdown — most informative for the full trajectory.
      if (h.days === HORIZONS[HORIZONS.length - 1].days) {
        lastHorizonDrivers = DIMENSIONS.map((d) => ({
          key: d.key,
          label: d.label,
          delta: Math.round(dimensionsFuture[d.key] - dimensionsToday[d.key]),
