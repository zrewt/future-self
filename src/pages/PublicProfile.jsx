...
const TIER_CONFIG = {
  ...
  legendary: { bg: 'rgba(124,58,237,0.12)', text: '#6626d9', border: 'rgba(124,58,237,0.25)' },
}
...
const PILLARS = [
  { key: 'fitness_score',   label: 'Fitness',   emoji: '🏋️', color: '#7c3aed' },
  { key: 'nutrition_score', label: 'Nutrition',  emoji: '🥗', color: '#00b8a0' },
  { key: 'energy_score',    label: 'Energy',     emoji: '💤', color: '#3b82c4' },
  { key: 'focus_score',     label: 'Focus',      emoji: '🎯', color: '#d97706' },
  { key: 'longevity_score', label: 'Longevity',  emoji: '🌿', color: '#e0527a' },
]
    ...
    return (
      <div className="app-bg min-h-screen flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="text-sm text-slate-500 dark:text-[#5A7050] font-medium mt-2">
          No Qyven user found for <span className="font-bold text-[#7c3aed] dark:text-[#00E87A]">@{username}</span>
        </p>
  ...
  return (
    <div className="app-bg min-h-screen">
      <div className="max-w-lg mx-auto px-4 py-8 pb-20 space-y-4">
        ...
        {/* ── Hero — gradient card, full brand treatment since this is the shareable card ── */}
        <div
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-28 h-28 rounded-full bg-white/10 translate-y-8 -translate-x-8 pointer-events-none" />

          {/* Profile header */}
          <div className="flex items-center gap-4 mb-5 relative z-10">
            <div
              className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl font-extrabold shadow-md shrink-0"
              style={{ background: isDark ? 'rgba(0,232,122,0.2)' : 'rgba(255,255,255,0.22)', color: isDark ? '#00E87A' : '#fff' }}
            >
              {initial}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-semibold mt-0.5" style={{ color: isDark ? '#9DB890' : 'rgba(255,255,255,0.8)' }}>
                Lv.{profile.level} · {levelName}
              </p>
              <p className="text-xs font-medium mt-0.5" style={{ color: isDark ? '#5A7050' : 'rgba(255,255,255,0.5)' }}>
          ...
          <div className="grid grid-cols-2 gap-2 relative z-10 mb-3">
            {[
              ...
              <div
                ...
                style={{ background: isDark ? 'rgba(0,232,122,0.08)' : 'rgba(255,255,255,0.14)' }}
              >
                <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: isDark ? '#5A7050' : 'rgba(255,255,255,0.6)' }}>
                  {s.label}
                </p>
                <p className="text-4xl font-extrabold tabular-nums leading-tight mt-0.5" style={{ color: isDark ? '#E8F0E0' : '#fff' }}>
                  {s.value}
                </p>
                <p className="text-xs font-bold mt-0.5" style={{ color: isDark ? '#9DB890' : 'rgba(255,255,255,0.75)' }}>
                  {s.sub}
                </p>
              </div>
            ))}
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[10px] font-bold uppercase" style={{ color: isDark ? '#5A7050' : 'rgba(255,255,255,0.6)' }}>Current streak</p>
                <p className="text-lg font-extrabold tabular-nums" style={{ color: isDark ? '#E8F0E0' : '#fff' }}>{profile.current_streak} days</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-bold uppercase" style={{ color: isDark ? '#5A7050' : 'rgba(255,255,255,0.6)' }}>Longest</p>
              <p className="text-lg font-extrabold tabular-nums" style={{ color: isDark ? '#E8F0E0' : '#fff' }}>{profile.longest_streak || 0} days</p>
            </div>
          </div>

          <p className="text-center text-[10px] font-bold uppercase tracking-widest mt-3 relative z-10" style={{ color: isDark ? '#5A7050' : 'rgba(255,255,255,0.4)' }}>
            qyven.vercel.app
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={handleShare}
          className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl font-semibold text-white transition-all dark:bg-gradient-to-br dark:from-[#00E87A] dark:to-[#7F5AF0]"
          style={{ background: isDark ? undefined : 'linear-gradient(135deg, #ff7ac6, #7c3aed, #00cdb4)', boxShadow: isDark ? undefined : '0 4px 18px rgba(124,58,237,0.3)' }}
        >
            ...
            <div style={{ width: '100%', height: 140 }}>
              <ResponsiveContainer>
                <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: -25 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke={isDark ? 'rgba(255,255,255,0.05)' : 'rgba(109,40,217,0.06)'} vertical={false} />
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(109,40,217,0.10)',
                    }}
                  >
        ...
        <div className={`${cardClass} p-6 text-center`}>
          ...
          <Link
            to="/signup"
            className="inline-flex px-8 py-3 text-base rounded-2xl font-bold text-white transition-all dark:bg-gradient-to-br dark:from-[#00E87A] dark:to-[#7F5AF0]"
            style={{ background: isDark ? undefined : 'linear-gradient(135deg, #ff7ac6, #7c3aed, #00cdb4)', boxShadow: isDark ? undefined : '0 4px 18px rgba(124,58,237,0.3)' }}
          >
            Start for free →
          </Link>
